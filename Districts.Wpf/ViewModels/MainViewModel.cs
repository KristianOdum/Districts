    ...
    ILogger<MainViewModel> logger) : INotifyPropertyChanged
{
    ...
    public async Task LoadDistrictsAsync()
    {
        ...
        Districts.Clear();
        foreach (var district in districts) Districts.Add(district);

        // Looks weird with nothing selected initially, so choose the first one if present
        if (Districts.Count > 0) SelectedDistrict = Districts[0];
    }
    ...
    public DistrictModel? SelectedDistrict
    {
        ...
        set
        {
            ...
            if (value is not null) _ = LoadDistrictDetailsAsync(value.Id);
        }
    }
    ...
    public async Task LoadSalespersonsAsync()
    {
        ...
        Salespersons.Clear();
        foreach (var salesperson in salespersons) Salespersons.Add(salesperson);
    }
    ...
    {
        if (SelectedDistrict is null) return;
